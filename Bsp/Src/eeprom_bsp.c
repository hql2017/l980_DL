...
//д�����ź�
#define  EEPROM_WC_ENABLE  HAL_GPIO_WritePin(EEROM_WC_OUT_GPIO_Port, EEROM_WC_OUT_Pin, GPIO_PIN_RESET)
#define  EEPROM_WC_DISABLE  HAL_GPIO_WritePin(EEROM_WC_OUT_GPIO_Port, EEROM_WC_OUT_Pin, GPIO_PIN_SET)
...
static HAL_StatusTypeDef EEPROM_M24C32_I2C_Read(unsigned short int regAddr,unsigned char *buff,unsigned short int len)
{
	...
	err=HAL_I2C_Mem_Read(&hi2c1,EEPROM_REAL_ADDR,regAddr,I2C_MEMADD_SIZE_16BIT,buff,len,EEPROM_I2C_TIMEOUT); 
	return err;	
}
/**
...
{
	HAL_StatusTypeDef err;
	err=HAL_I2C_Mem_Write(&hi2c1,EEPROM_REAL_ADDR,regAddr,I2C_MEMADD_SIZE_16BIT,buff,len,EEPROM_I2C_TIMEOUT);  
	return err; 
}
